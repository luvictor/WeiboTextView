...
public class MainActivity extends Activity {
		...
		et_content.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				for (int i = 0; i < mAtList.size(); i++) {
					...
					if (lastPos != -1) {
						if (selectionStart >= lastPos && selectionStart <= (lastPos + at.length())) {
							// 在这position 区间就移动光标
	...
	@Subscribe
	public void onGetAtPeople(String at) {
		Toast.makeText(getApplicationContext(), "接收到", Toast.LENGTH_SHORT).show();
		mAtList.add("@" + at);
		et_content.append("@" + at);
